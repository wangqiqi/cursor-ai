...
> 蒸馏自社区 visual QA / verifying-in-browser 思路；**不新建** skill。深挖 a11y → 候选 Sprint / 后续 §。
...
- [ ] **轻量 a11y** — 主流程可键盘到达主按钮；明显缺 label 的控件记 Medium+（完整审计留给后续）
- [ ] **跳过声明** — 若未跑本节：报告写「§10 skipped: …」

UI/功能 Sprint **建议** 在 **`/release`** 前勾选 §8；含导入/恢复时勾选 §9；有可开页面时勾选 §10；纯后端可省略并声明。
...
| **delivery** | 7 维走查 · 可选 §8–§10 · 冲突上报 | 不写业务代码（除非用户 steering 修 Blocker） |
